...
class UnifiedEmployeeDataGenerator:
    """
    Single entrypoint for synthetic employee wellbeing / burnout data.
    Versioned so you can compare models across distributions.
    """
    ...
    def _generate_v1(self) -> pd.DataFrame:
        ...
        # simple burnout risk score (only to define labels)
        burnout_score = (
            0.15 * (df["WorkHoursPerWeek"] - 40)
            + 0.3 * (df["StressLevelSelfReport"] - 5)
            - 0.2 * (df["ManagerSupportScore"] - 3)
            - 0.15 * (df["RecognitionFrequency"] - 2)
            + 0.1 * (df["FinancialStressScore"] - 3)
            - 0.1 * (df["RemoteDaysPerWeek"] - 2)
        )

        burnout_score = burnout_score + RNG.normal(0, 1.0, size=len(df))
        df["BurnoutScore"] = burnout_score

        # label: high burnout risk – model will learn this pattern
        df["HighBurnoutRisk"] = (
    ...
    # --- Version 2: more nuanced, persona-aware ---
    def _generate_v2(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({"EmployeeID": np.arange(1, n + 1), "Persona": personas})

        def sample_from_persona(row_idx, persona):
        ...
        for i, persona in enumerate(personas):
            rows.append(sample_from_persona(i, persona))
        ...
        # ----- RULE-BASED RISK (ONLY FOR LABELS, NOT USED IN UI) -----
        rule_raw = (
        ...
        # Logistic squashing to 0–1 just to get a smooth "intensity"
        rule_prob = 1 / (1 + np.exp(-0.4 * rule_raw))

        # Label for training the ML model
        df["HighBurnoutRisk"] = (rule_prob > 0.6).astype(int)

        # ---- Work-Life Balance & Job Satisfaction (1–4) ----
        # Link them to rule_prob so data is coherent, but this prob is NEVER shown in the UI.
